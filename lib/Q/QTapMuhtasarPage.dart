...
class QTapMuhtasarPage extends StatefulWidget {
  QTapMuhtasarPage({Key key}) : super(key: key);
  @override
  State<QTapMuhtasarPage> createState() => _QTapMuhtasarPageState();
}

class _QTapMuhtasarPageState extends State<QTapMuhtasarPage>
  ...
  showOpenSaved() async {
    int suraIDs = await q.setting.getSettingInt("muhtasarCsuraID", suraID);
    int ayas = await q.setting.getSettingInt("muhtasarCaya", aya);
  ...
  upadateAyaList() async {
    tapsir = await q.sql.selectOne(
        'SELECT id,sura,aya,text,ug_text,(SELECT ar_text FROM a_tapsir_mukhtasar where sura=$suraID and aya=$aya) as ar_text,(SELECT en_text FROM a_tapsir_mukhtasar where sura=$suraID and aya=$aya) as en_text,(SELECT cn_text FROM a_tapsir_mukhtasar where sura=$suraID and aya=$aya) as cn_text,(SELECT tr_text FROM a_tapsir_mukhtasar where sura=$suraID and aya=$aya) as tr_text FROM a_Q_text where sura=$suraID and aya=$aya');
    ...
    fav = q.favMuhtasar.isQFavMuhtasar(suraID * 1000 + aya);
    setState(() {});
                  ...
                  children: [
                    ...
                    IconButton(
                        onPressed: () {
                          ...
                          if (!fav) {
                            q.favMuhtasar
                                .removeQFavMuhtasar(suraID * 1000 + aya);
                          } else {
                            q.favMuhtasar.addQFavMuhtasar(suraID * 1000 + aya);
                          }
                        },
                        icon: Icon(
                          Icons.book_rounded,
                    ...
                    IconButton(
                        onPressed: () {
                          q.setting.saveSettingInt("muhtasarCsuraID", suraID);
                          q.setting.saveSettingInt("muhtasarCaya", aya);
  ...
  showbookmarks() {
    if (q.favMuhtasar.qFavMuhtasar.length == 0) {
      q.toast(q.resources.saklangalaryok);
    } else {
      List<Map> favsura = [];
      List<int> ss = [];
      for (int i = 0; i < q.favMuhtasar.qFavMuhtasar.length; i++) {
        int sid = q.favMuhtasar.qFavMuhtasar[i];
        sid = (sid ~/ 1000).toInt();
        if (!ss.contains(sid)) {
          ss.add(sid);
        }
  ...
  updateAyas(int id) {
    ...
    for (int i = 0; i < q.favMuhtasar.qFavMuhtasar.length; i++) {
      ...
      if (qTFav.suraid.value == sid) {
        int aya = q.favMuhtasar.qFavMuhtasar[i] - sid * 1000;
