...
class QTapJalaleinPage extends StatefulWidget {
  QTapJalaleinPage({Key key}) : super(key: key);
  @override
  State<QTapJalaleinPage> createState() => _QTapJalaleinPageState();
}

class _QTapJalaleinPageState extends State<QTapJalaleinPage>
  ...
  showOpenSaved() async {
    int suraIDs = await q.setting.getSettingInt("JalaleinCsuraID", suraID);
    int ayas = await q.setting.getSettingInt("JalaleinCaya", aya);
  ...
  upadateAyaList() async {
    tapsir = await q.sql.selectOne(
        'SELECT id,sura,aya,text,ug_text,(SELECT ar_jalalein FROM a_tapsir where sura=$suraID and aya=$aya) as ar_jalalein FROM a_Q_text where sura=$suraID and aya=$aya');
    ...
    fav = q.favJalalein.isQFavJalalein(suraID * 1000 + aya);
    setState(() {});
                  ...
                  children: [
                    ...
                    IconButton(
                        onPressed: () {
                          ...
                          if (!fav) {
                            q.favJalalein
                                .removeQFavJalalein(suraID * 1000 + aya);
                          } else {
                            q.favJalalein.addQFavJalalein(suraID * 1000 + aya);
                          }
                        },
                        icon: Icon(
                          Icons.book_rounded,
                    ...
                    IconButton(
                        onPressed: () {
                          q.setting.saveSettingInt("JalaleinCsuraID", suraID);
                          q.setting.saveSettingInt("JalaleinCaya", aya);
  ...
  showbookmarks() {
    if (q.favJalalein.qFavJalalein.length == 0) {
      q.toast(q.resources.saklangalaryok);
    } else {
      List<Map> favsura = [];
      List<int> ss = [];
      for (int i = 0; i < q.favJalalein.qFavJalalein.length; i++) {
        int sid = q.favJalalein.qFavJalalein[i];
        sid = (sid ~/ 1000).toInt();
        if (!ss.contains(sid)) {
          ss.add(sid);
        }
            ...
            return AlertDialog(
              ...
              content: Container(
                  ...
                  child: Container(
                    ...
                    child: Row(textDirection: q.theme.dir, children: [
                      Expanded(
                        child: Container(
                          width: double.infinity,
                          alignment: Alignment.center,
  ...
  updateAyas(int id) {
    ...
    for (int i = 0; i < q.favJalalein.qFavJalalein.length; i++) {
      ...
      if (qTFav.suraid.value == sid) {
        int aya = q.favJalalein.qFavJalalein[i] - sid * 1000;
