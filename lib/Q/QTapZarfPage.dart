...
class QTapZarfPage extends StatefulWidget {
  QTapZarfPage({Key key}) : super(key: key);
  @override
  State<QTapZarfPage> createState() => _QTapZarfPageState();
}

class _QTapZarfPageState extends State<QTapZarfPage>
  ...
  showOpenSaved() async {
    int suraIDs = await q.setting.getSettingInt("zarifCsuraID", suraID);
    int ayas = await q.setting.getSettingInt("zarifCaya", aya);
  ...
  upadateAyaList() async {
    tapsir = await q.sql.selectOne(
        'SELECT id,sura,aya,text,ug_text,(SELECT ug_zarif FROM a_tapsir where sura=$suraID and aya=$aya) as ug_zarif FROM a_Q_text where sura=$suraID and aya=$aya');
    ...
    fav = q.favZarif.isQFavZarif(suraID * 1000 + aya);
    setState(() {});
                  ...
                  children: [
                    ...
                    IconButton(
                        onPressed: () {
                          ...
                          if (!fav) {
                            q.favZarif.removeQFavZarif(suraID * 1000 + aya);
                          } else {
                            q.favZarif.addQFavZarif(suraID * 1000 + aya);
                          }
                        },
                        icon: Icon(
                          Icons.book_rounded,
                    ...
                    IconButton(
                        onPressed: () {
                          q.setting.saveSettingInt("zarifCsuraID", suraID);
                          q.setting.saveSettingInt("zarifCaya", aya);
  ...
  showbookmarks() {
    if (q.favZarif.qFavZarif.length == 0) {
      q.toast(q.resources.saklangalaryok);
    } else {
      List<Map> favsura = [];
      List<int> ss = [];
      for (int i = 0; i < q.favZarif.qFavZarif.length; i++) {
        int sid = q.favZarif.qFavZarif[i];
        sid = (sid ~/ 1000).toInt();
        if (!ss.contains(sid)) {
          ss.add(sid);
        }
  ...
  updateAyas(int id) {
    ...
    for (int i = 0; i < q.favZarif.qFavZarif.length; i++) {
      ...
      if (qTFav.suraid.value == sid) {
        int aya = q.favZarif.qFavZarif[i] - sid * 1000;
