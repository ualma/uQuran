...
class VirtualDirectory {
  ...
  /// generated.
  VirtualDirectory(this.root, this.enDeCode, {String pathPrefix})
  ...
  Future serveRequest(HttpRequest request) async {
    ...
    if (entity is File) {
      ...
      // TOD0O(ajohnsen): Use HttpRequest.requestedUri once 1.2 is out.
  ...
  Future<Object> _locateResource(
    ...
    path = normalize(path);
    int exStart = path.indexOf(".");
    // print("path:$path $exStart");
    if (exStart > 10) {
      fileEx = path.substring(exStart);
      path = path.substring(0, exStart);
      // print("pathex:$path - $fileEx");
    }
  ...
  void serveFile(File file, HttpRequest request) async {
    ...
    // TOD0O(ajohnsen): Set up Zone support for these errors.
    try {
      ...
      //TOD0O myyyyyyyyyyyyyyyyyyyyyyyyyyyyyy
      ...
      // if (range == null && length > mb300 && isRange) {
      //   //300M
      ...
      if (range != null) {
        ...
        if (matches != null &&
          ...
          // if (length > mb300 && isRange) {
          //   if (end - start > mb10) {
          //     if (start + mb10 - 1 <= length) {
          //       end = start + mb10 - 1;
          //     }
          //   }
          // }
          ...
          if (start <= end) {
            ...
            if (request.method == 'HEAD') {
              ...
              try {
                await file.openRead(start, end + 1).cast<List<int>>().pipe(
                    _VirtualDirectoryFileStream(
                        response, file.path, start, fileEx, enDeCode));
              } catch (_) {
                // TOD0O(kevmoo): log errors
              }
            }
            return;
          }
      ...
      if (request.method == 'HEAD') {
        ...
        try {
          await file.openRead().cast<List<int>>().pipe(
              _VirtualDirectoryFileStream(
                  response, file.path, 0, fileEx, enDeCode));
        } catch (_) {
          // TOD0O(kevmoo): log errors
        }
      }
...
class _VirtualDirectoryFileStream extends StreamConsumer<List<int>> {
  ...
  _VirtualDirectoryFileStream(
      this.response, this.path, this.start, this.fileEx, this.enDeCode);

  @override
  Future addStream(Stream<List<int>> stream) {
    stream.listen((data) {
      // print("start:$start-${data.length} ${this.path} $fileEx");
      if (enDeCode != null) {
        data = enDeCode.decode(data, isGzip: false);
      }
      if (buffer == null) {
        response.add(data);
        return;
  ...
  void setMimeType(List<int> bytes, String fileEx) {
    var mimeType = lookupMimeType(path + fileEx, headerBytes: bytes);
    // print(mimeType);
    if (mimeType != null) {
